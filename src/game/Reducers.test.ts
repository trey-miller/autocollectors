...
describe('Decrement works', () => {
    test('Proper block is decremented.', () => {
        ...
        const newState = rootReducer(state, { type: 'DECREMENT', payload: state.blocks[1][2] });
    ...
    test('Block stuff does not go below zero', () => {
        ...
        const newState = rootReducer(state, { type: 'DECREMENT', payload: state.blocks[y][x] });
        expect(newState.blocks[y][x].stuff).toEqual(0);
    });
    test('Reducer does not change number of rows or columns', () => {
        const state = createDefaultState();
        const newState = rootReducer(state, { type: 'DECREMENT', payload: state.blocks[0][0] });
